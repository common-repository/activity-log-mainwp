...
class View extends Abstract_View {
	...
	public function __construct() {
		...
		if ( MWPAL_Extension\mwpal_extension()->is_mainwp_active() ) {
			if ( \version_compare( \MainWP_System::$version, '4.0-beta', '<' ) ) {
				...
				if ( 'dark' == get_user_meta( $user_id, 'wp_mainwp_selected_theme', true ) ) {
					add_action( 'admin_print_styles', array( $this, 'admin_print_dark_styles' ) );
				}
			}
		}
	...
	public function admin_print_dark_styles() {
		// Global WP page now variable.
		global $pagenow;
		...
		?>
		<style>
		.mainwp-ui.mainwp-custom-theme .almwp-table .alternate, .mainwp-ui.mainwp-custom-theme .striped.almwp-table > tbody > :nth-child(2n+1) *:not(a) {
			color: #d0cfcf
		}

		.mainwp-ui.mainwp-custom-theme .mwpal-content-wrapper .postbox {
			background: #2b2b2b;
			border: none;
			color: #fff
		}

		.mainwp-ui.mainwp-custom-theme .mwpal-content-wrapper .postbox p.description {
		color: #fff;
		padding-left: 12px;
		}

		.mainwp-ui.mainwp-custom-theme .mwpal-content-wrapper .postbox h2 {
		padding-left: 12px;
		}

		.mainwp-ui.mainwp-custom-theme .sites-container {
	...
	public function enqueue_styles() {
		...
		if ( in_array( $this->current_tab, array( 'activity-log', Enforce_Settings_View::$tab_id ) ) ) {
			// Select2 styles.
			wp_enqueue_style(
				'mwpal-select2-css',
	...
	public function tab_settings() {
		...
		<div class="metabox-holder columns-1">
			<form method="post" id="mwpal-settings">
							...
							<table class="form-table">
								...
								<tr>
									...
									<td>
										<fieldset>
											<label for="pruning-enabled">
												<input type="button" class="ui green fluid button" name="events-pruning-now" id="purge-trigger" value="<?php esc_html_e( 'Delete activity log', 'mwp-al-ext' ); ?>">
											</label>
										</fieldset>
									</td>
								</tr>
				...
				<p class="submit">
					<input type="submit" name="submit" id="submit" class="ui green fluid button button-hero" style="width:auto" value="<?php esc_attr_e( 'Save Settings', 'mwp-al-ext' ); ?>">
				</p>
			</form>
		</div>
		<?php
	...
	public function detect_view_type() {
		...
		if ( defined( 'DOING_AJAX' ) ) {
			...
			$requested_view = ( isset( $_REQUEST['view'] ) ) ? \sanitize_text_field( \wp_unslash( $_REQUEST['view'] ) ) : '';
		}
	...
	public function refresh_child_sites() {
		...
		$run_id = ( isset( $_POST['mwpal_run_id'] ) ) ? \sanitize_text_field( \wp_unslash( $_POST['mwpal_run_id'] ) ) : uniqid();
		$forced = ( isset( $_POST['mwpal_forced'] ) ) ? \sanitize_text_field( \wp_unslash( $_POST['mwpal_forced'] ) ) : false;
	...
	public function managesites_activity_logs() {
		 ...
		 */
		add_filter(
			'pre_option_' . MWPAL_OPT_PREFIX . 'wsal-child-sites',
			function() {
				return array();
			}
		);
	...
	public function user_notice_dismissed() {
		...
		$notice_type       = \sanitize_text_field( \wp_unslash( $_POST['notice'] ) );
	...
	public static function render_sites_selection_ui( $all_sites, $left_pane_text, $left_pane_sites, $right_pane_text, $right_pane_sites, $wrap = false, $add_button_text = '', $show_refresh_button = true, $disable_ajax = false, $init_hidden = false ) {
		...
		<div class="mwpal-wcs-container">
			...
			<?php
			if ( ! empty( $disable_ajax ) ) :
				?>
				data-disable-ajax="yes"<?php endif; ?>>
				<a href="javascript:;" class="ui green fluid button" id="mwpal-wcs-add-btn"><?php echo $add_button_text; ?> <span class="dashicons dashicons-arrow-right-alt2"></span></a>
				<br>
				<a href="javascript:;" class="ui fluid button" id="mwpal-wcs-remove-btn"><span class="dashicons dashicons-arrow-left-alt2"></span> <?php esc_html_e( 'Remove', 'mwp-al-ext' ); ?></a>
			</div>
			<div id="mwpal-wcs-al">
		...
		<input type="button" class="ui green fluid button" style="width:auto" id="mwpal-wsal-sites-refresh" value="<?php esc_html_e( 'Refresh list of child sites', 'mwp-al-ext' ); ?>" data-title="<?php esc_html_e( 'Refresh list of child sites', 'mwp-al-ext' ); ?>" />
