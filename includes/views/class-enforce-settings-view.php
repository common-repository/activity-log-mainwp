...
class Enforce_Settings_View {
	...
	public function render() {
		...
		$sites_with_enforced_settings = array();
		...
		$selected_events = ( array_key_exists( 'disabled_events', $enforced_settings ) && ! empty( $enforced_settings['disabled_events'] ) ) ? array_map( 'intval', explode( ',', $enforced_settings['disabled_events'] ) ) : array();
				...
				<tr>
					...
					<td>
						<fieldset>
							...
							<?php
							$sites_with_enforced_settings_mapped = array();
	...
	private function maybe_notify_child_sites( $previous_where_to_enforce_settings, $where_to_enforce_settings, $previously_enforced_settings, $settings_to_enforce, $previous_list_of_subsites, $child_sites_to_enforce ) {
		...
		switch ( $previous_where_to_enforce_settings ) {
			case 'all':
				...
				} elseif ( 'some' == $where_to_enforce_settings ) {
				...
				} elseif ( 'all' == $where_to_enforce_settings && $enforced_settings_changed ) {
	...
	public function enqueue_scripts() {
		...
		wp_localize_script(
			$script_handle,
			'mwpal_enforced_settings',
			array(
				'events'       => json_encode( MWPAL_Extension\Helpers\DataHelper::get_events_for_select2() ),
